...
namespace CoffeeShopWithOperation.CustomerRepository
{
   public class CustomersRepository
    {
        //public bool additems(string name, string address,string contract)
        public bool additems(Customer customer)
        {
            bool isaded = false;
            try
            {
                ...
                string itemsqurey = @" INSERT INTO  Customer (Name,Address,Contract) VALUES('" + customer.Name + "','" + customer.Address + "','"+customer.Contract+"')";
        ...
        public bool update(Customer customer)
        {
            try
            {
                ...
                string sqlvalues = @"UPDATE Customer SET Name='" + customer.Name + "',Address='" + customer.Address + "',Contract='"+customer.Contract+"' WHERE ID=" + customer.ID + "";
                SqlCommand sqlCommand = new SqlCommand(sqlvalues, sqlConnection);
                sqlConnection.Open();
...

        {
            try
            {
                ...
                string sqlvalues = @"DELETE FROM Customer WHERE ID=" + customer.ID + "";
                SqlCommand sqlCommand = new SqlCommand(sqlvalues, sqlConnection);
                sqlConnection.Open();
                int isexcuted = sqlCommand.ExecuteNonQuery();
                if (isexcuted > 0)
        ...
        public DataTable search(Customer customer)
        {
            ...
            string sqlvalues = @"select*from  Customer  where name='" + customer.Name + "'";
            SqlCommand sqlCommand = new SqlCommand(sqlvalues, sqlConnection);
